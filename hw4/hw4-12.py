...
11. Next, consider the following homogeneous order-Q polynomial transform: Φ(x) = (1, x1 , x2 , ..., x12 , x1^2 , x2^2 , ..., x12^2 , ..., x1^Q , x2^Q , ..., x12^Q ). Transform the training and testing data according to Φ(x) with Q = 3, and again implement steps 1-4 in the previous problem on z_n = Φ(x_n)instead of x_n to get w_poly. Repeat the four steps above 1126 times, each with a different random seed. Plot a histogram of Ein^sqr(w_lin) - Ein^sqr(w_poly).What is the average Ein^sqr(w_lin) - Ein^sqr(w_poly)? This is the Ein gain for using (homogeneous) polynomial transform. 

12. Following from the 1126 experiments in the previous problem, plot a histogram of Eout^sqr(w_lin) - Eout^sqr(w_poly). What is the average Eout^sqr(w_lin) - Eout^sqr(w_poly)? This is the Eout change for using (homogeneous) polynomial transform.
'''
...
def main():
    ...
    for _ in range(num_experiments):
        ...
        Eout_lin = squared_error(y_out, X_out @ w_lin)
        ...
        Eout_poly = squared_error(y_out, Z_out @ w_poly)

        # Store Eout values for comparison
        Eout_lin_list.append(Eout_lin)
        Eout_poly_list.append(Eout_poly)

    # Calculate difference in Eout
    Eout_diff = np.array(Eout_lin_list) - np.array(Eout_poly_list)

    # Plot histogram of Eout differences
    plt.figure(figsize=(10, 6))
    plt.hist(Eout_diff, bins=30, edgecolor='black')
    plt.xlabel(r'$E_{\text{out}}^{\text{sqr}}(\mathbf{w}_{\text{lin}}) - E_{\text{out}}^{\text{sqr}}(\mathbf{w}_{\text{poly}})$')
    plt.ylabel("Number of Experiments")
    plt.title(r'Histogram of $E_{\text{out}}^{\text{sqr}}(\mathbf{w}_{\text{lin}}) - E_{\text{out}}^{\text{sqr}}(\mathbf{w}_{\text{poly}})$')
    plt.show()

    # Calculate and print average difference
    avg_Eout_diff = np.mean(Eout_diff)
    print(f"Average Eout(w_lin) - Eout(w_poly): {avg_Eout_diff}")
