...
12. Repeat Problem 11, but using the first 2 features for each example instead of all 12 features. That is, run linear regression with x = [x0,x1,x2] instead. In particular, compare your results here to those of Problem 11.
'''
...
def download_and_parse_data():
	...
	for line in lines:
		...
		feature = [1]  # x0 = 1
		for token in tokens[1:3]: # Use only the first 2 features
			_, value = token.split(":")
